...
export function setupDslLanguage(monaco: Monaco) {
  const already = monaco.languages
    .getLanguages()
    .some((l: { id: string }) => l.id === DSL_LANG);
  if (already) return;
  ...
  monaco.languages.setMonarchTokensProvider(DSL_LANG, {
    tokenizer: {
      root: [
        [/(\/\/|#).*$/, "comment"],
        [/"[^"]*"/, "string"],
        // flow + sequence keywords
        [
          /\b(direction|title|participant|actor|note|over|as|left of|right of)\b/,
          "keyword",
        ],
        // arrows: sequence async (-->>, ->>) longest-first, then flow set
        [/-->>|->>|-->|<>|--|>|<|-/, "operator"],
