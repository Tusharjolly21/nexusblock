 ...
 * center while scroll scrubs a camera (zoom/pan) and animates the UI *inside*
 * the mockup — connectors draw, docs type, tabs switch, AI generates, cursors
 * collaborate. Every beat is demonstrated, not screenshotted.
 *
...
const SCENES = [
  { tab: 'canvas', name: 'Infinite canvas', body: 'Drop nodes, connect them with smart elbow + arrow routing.', icon: 'lucide:pen-tool' },
  { tab: 'doc', name: 'Docs beside diagrams', body: 'Write notes and embed the live canvas as a figure — never a stale screenshot.', icon: 'lucide:file-text' },
  { tab: 'code', name: 'Diagram as code', body: 'Write flow / ERD DSL that compiles straight to native shapes.', icon: 'lucide:file-code-2' },
  { tab: 'ai', name: 'AI + live collaboration', body: 'Generate from a prompt, then review together with live cursors and comments.', icon: 'lucide:sparkles' },
] as const

// Scene N owns progress [n*0.25, (n+1)*0.25]; helpers fade/scrub within it.
const R = (i: number): [number, number] => [i * 0.25, (i + 1) * 0.25]
...
export function CinematicShowcase() {
  ...
  const scale = useTransform(p, [0, 0.22, 0.5, 0.78, 1], [1, 1.05, 1.05, 1.04, 0.99])
  const x = useTransform(p, [0, 0.25, 0.5, 0.75, 1], ['0%', '5%', '-7%', '0%', '0%'])
  ...
  return (
    <section ref={ref} className="relative" style={{ height: '460vh' }}>
      <div className="sticky top-0 flex h-[100svh] items-center justify-center overflow-hidden px-6">
        {/* depth: soft radial spotlight behind the frame */}
...
/** Left-side chapter labels that crossfade as scenes change. */
  ...
  return (
    <div className="pointer-events-none absolute left-6 top-1/2 z-20 hidden w-[248px] -translate-y-1/2 lg:block xl:left-[max(24px,calc(50%-620px))]">
      {SCENES.map((s, i) => {
        ...
        const opacity = useTransform(p, [a - 0.03, a + 0.05, b - 0.05, b + 0.03], [0, 1, 1, 0])
        // eslint-disable-next-line react-hooks/rules-of-hooks
        const y = useTransform(p, [a - 0.03, a + 0.05], [16, 0])
        return (
          <motion.div key={s.name} style={{ opacity, y }} className="absolute inset-x-0">
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-line bg-surface px-3 py-1.5 font-mono text-[10px] uppercase tracking-widest text-grey-3">
              <Icon icon={s.icon} width={12} /> {String(i + 1).padStart(2, '0')} / 04
            </div>
...
function Tab({ p, label, active }: { p: MotionValue<number>; label: string; active: number[] }) {
  ...
  return (
    <span className="relative rounded-md px-2.5 py-1 text-[11px] font-medium text-grey-4">
      ...
      <span className="relative" style={{ mixBlendMode: 'normal' }}>
        <motion.span style={{ color: useTransform(on, [0, 1], ['var(--color-grey-4)', 'var(--color-paper)']) }}>{label}</motion.span>
      </span>
    </span>
  )
}
...
function AppFrame({ p }: { p: MotionValue<number> }) {
  ...
  const o1 = useTransform(p, [0, 0.03, 0.22, 0.27], [1, 1, 1, 0.15])
  const o2 = useTransform(p, [0.24, 0.29, 0.47, 0.52], [0, 1, 1, 0])
  const o3 = useTransform(p, [0.49, 0.54, 0.72, 0.77], [0, 1, 1, 0])
  const o4 = useTransform(p, [0.74, 0.79, 0.98, 1], [0, 1, 1, 1])

  // Scene 1 — connectors draw.
  const draw1 = useTransform(p, [0.03, 0.2], [0, 1])
  // Scene 2 — doc wipe + figure drop.
  const docWipe = useTransform(p, [0.29, 0.44], ['inset(0 100% 0 0)', 'inset(0 0% 0 0)'])
  const figOpacity = useTransform(p, [0.42, 0.5], [0, 1])
  const figY = useTransform(p, [0.42, 0.5], [14, 0])
  // Scene 3 — code wipe + applied.
  const codeWipe = useTransform(p, [0.54, 0.68], ['inset(0 100% 0 0)', 'inset(0 0% 0 0)'])
  const appliedO = useTransform(p, [0.68, 0.73], [0, 1])
  // Scene 4 — prompt wipe, generate, cursors.
  const promptWipe = useTransform(p, [0.79, 0.88], ['inset(0 100% 0 0)', 'inset(0 0% 0 0)'])
  const genO = useTransform(p, [0.88, 0.94], [0, 1])
  const genScale = useTransform(p, [0.88, 0.94], [0.85, 1])
  const curAx = useTransform(p, [0.8, 1], ['12%', '58%'])
  const curAy = useTransform(p, [0.8, 1], ['62%', '30%'])
  const curBx = useTransform(p, [0.8, 1], ['70%', '34%'])
  const curBy = useTransform(p, [0.8, 1], ['28%', '66%'])
  const collabO = useTransform(p, [0.82, 0.9], [0, 1])
    ...
    <div className="overflow-hidden rounded-[22px] border border-line bg-surface shadow-[0_50px_120px_-40px_rgba(15,23,42,.4)]">
      ...
      <div className="flex items-center gap-3 border-b border-line bg-paper px-4 py-2.5">
        ...
        <div className="ml-2 flex items-center gap-1">
          <Tab p={p} label="Canvas" active={[0, 3]} />
      ...
      <div className="relative h-[clamp(320px,52vh,520px)] overflow-hidden" style={{ backgroundImage: 'radial-gradient(circle, var(--color-canvas-dot) 1px, transparent 1px)', backgroundSize: '22px 22px' }}>
        {/* Base canvas nodes (shared spatial anchor across scenes) */}
        <div className="absolute inset-0">
          ...
          <motion.div style={{ opacity: genO, scale: genScale }}>
            <StageNode x="56%" y="74%" icon="logos:redis" label="Cache" />
          </motion.div>
        </div>

        {/* Scene 2 — doc panel slides over from the right */}
        ...
        {/* Scene 3 — code editor slides over from the right */}
...
"Events"  > "Ledger": write`}
        ...
        {/* Scene 4 — AI prompt + collaboration overlay */}
        ...
        <motion.div style={{ opacity: o1 }} className="pointer-events-none absolute right-4 top-4 flex items-center gap-1.5 rounded-full border border-line bg-surface/90 px-2.5 py-1 font-mono text-[9.5px] text-grey-4 shadow-sm backdrop-blur">
...
/** Calm, motion-free version for reduced-motion users. */
function StaticFallback() {
  return (
    <section className="mx-auto max-w-6xl px-6 py-24">
      <h2 className="mb-8 max-w-[16ch] font-display text-[clamp(30px,4vw,50px)] font-medium tracking-[-0.03em]">A full canvas, docs, code, and AI workspace.</h2>
